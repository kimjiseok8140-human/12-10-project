...
public:
  void Load(physics::ModelPtr model, sdf::ElementPtr sdf) override
  {
    ...
    // Robot name for attaching
    robot_name_ = sdf->HasElement("robot_name") ? sdf->Get<std::string>("robot_name") : "waffle";
...
private:
  ...
  void OnCmdFloor(const std_msgs::msg::Int32::SharedPtr msg)
  {
    ...
    RCLCPP_INFO(ros_node_->get_logger(), "Received CmdFloor: %d", f);
  ...
  void OnUpdate()
  {
    ...
    // Handle Robot Attachment
    HandleRobotAttachment();
...
private:
  ...
  // --- Robot Attachment Members ---
  std::string robot_name_{"waffle"};
  physics::JointPtr attach_joint_;
  bool is_attached_{false};

  void HandleRobotAttachment()
  {
      if (!arrived_ && !is_attached_) {
          // Moving and not attached -> Try to attach
          if (CheckRobotOnElevator()) {
              AttachRobot();
          }
      } else if (arrived_ && is_attached_) {
          // Arrived and attached -> Detach
          DetachRobot();
      }
  }

  bool CheckRobotOnElevator()
  {
      auto robot = world_->ModelByName(robot_name_);
      if (!robot) return false;

      // Get platform pose
      auto platform_link = model_->GetLink("platform");
      if (!platform_link) return false;
      
      auto plat_pose = platform_link->WorldPose();
      auto robot_pose = robot->WorldPose();

      // Simple bounds check (Platform is approx 2.7x2.7)
      double dx = std::abs(plat_pose.Pos().X() - robot_pose.Pos().X());
      double dy = std::abs(plat_pose.Pos().Y() - robot_pose.Pos().Y());
      double dz = robot_pose.Pos().Z() - plat_pose.Pos().Z();

      // Check if robot is "on top" (dx, dy within 1.35, dz positive and small)
      if (dx < 1.3 && dy < 1.3 && dz > 0.0 && dz < 0.5) {
          return true;
      }
      return false;
  }
  ...
  void AttachRobot()
  {
      ...
      if (!robot) return;
      auto platform_link = model_->GetLink("platform");
      auto robot_link = robot->GetLink("base_footprint"); // attached to base_footprint
      if (!robot_link) robot_link = robot->GetLink("base_link"); // fallback
      if (!platform_link || !robot_link) return;

      RCLCPP_INFO(ros_node_->get_logger(), "Attaching robot [%s] to elevator platform", robot_name_.c_str());

      attach_joint_ = world_->Physics()->CreateJoint("fixed", model_);
      attach_joint_->SetName("elevator_robot_attachment");
      attach_joint_->Attach(platform_link, robot_link);
      attach_joint_->Load(platform_link, robot_link, ignition::math::Pose3d()); // Use ignition math for recent Gazebo
      attach_joint_->Init();
      is_attached_ = true;
  }
  ...
  {
      if (attach_joint_) {
          RCLCPP_INFO(ros_node_->get_logger(), "Detaching robot [%s] from elevator platform", robot_name_.c_str());
